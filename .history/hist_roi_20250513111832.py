...
# Save image showing only the tumor (masked region, background zero)
slice_idx = img_data.shape[2] // 2  # <-- Make sure this is defined before using it

tumor_only = np.zeros_like(img_data)
tumor_only[roi_mask] = img_data[roi_mask]
tumor_only_slice = tumor_only[:, :, slice_idx]

plt.figure(figsize=(6, 6))
plt.imshow(tumor_only_slice, cmap='gray')
plt.title('Tumor Only (Masked Region)', fontsize=14)
plt.axis('off')
tumor_only_path = os.path.join(results_dir, "tumor_only_masked_middle_slice.png")
