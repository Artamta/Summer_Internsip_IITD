...
# Display and save some random image slices from the last volume
np.random.seed(42)
random_slices = np.random.choice(last_vol.shape[2], size=2, replace=False)
for idx, rand_slice in enumerate(random_slices):
    plt.figure(figsize=(6, 6))
    plt.imshow(last_vol[:, :, rand_slice], cmap='gray')
    plt.title(f'Random Slice {rand_slice} (Last Volume)')
    plt.axis('off')
    rand_path = os.path.join(results_dir, f"ivim_chest_last_b9_random_slice_{rand_slice}.png")
    plt.savefig(rand_path)
    plt.close()
    print(f"Saved random image slice to {rand_path}")

print("Saved all requested and random slices to Results folder.")